# -*- coding: utf-8 -*-
"""
Created on Sun Sep 12 13:03:22 2021
...
# Plot the raw data as well as the true function (without noise)
...
# To visualize each learned model
...
def radial_basis_transform(X, B, gamma=0.1):
    ...
    for i in range(len(X)):
        for j in range(len(B)):
            Phi[i][j] = np.exp((-1) * gamma * (X[i] - B[j])**2)
    return Phi
...
for lam in l:  # Iterate over polynomial degree
    
    Phi_trn = radial_basis_transform(X_trn, X_trn, lam)                 # Transform training data into d dimensions
    w[lam] = train_ridge_model(Phi_trn, y_trn, lam)                       # Learn model on training data
    
    Phi_val = radial_basis_transform(X_val, X_trn, lam)                 # Transform validation data into d dimensions
    validationErr[lam] = evaluate_model(Phi_val, y_val, w[lam])  # Evaluate model on validation data
    
    Phi_tst = radial_basis_transform(X_tst, X_trn, lam)           # Transform test data into d dimensions
...
for lam in l:
    X_lam = radial_basis_transform(x_true, X_trn, lam)
    y_lam = X_lam @ w[lam]
    plt.plot(x_true, y_lam, marker='None', linewidth=2)

plt.legend(['true'] + list(l))
plt.axis([-8, 8, -15, 15])